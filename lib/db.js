...
export async function confirmHold({ dateISO, bayId, startMin, endMin, patch }) {
  ...
  const flip = (p) => db.from('bookings')
    .update({ ...p, status: 'confirmed', expires_at: null })
    .eq('booking_date', dateISO).eq('bay_id', bayId).eq('start_min', startMin).eq('end_min', endMin)
    .eq('status', 'held')
    .select('id');
  let { data, error } = await flip(patch);
  // A database still on 0033 has no hold columns. Save the booking without them rather than
  // leaving a paid or authorised customer with no slot (see withoutHoldColumns).
  if (holdSchemaError(error)) ({ data, error } = await flip(withoutHoldColumns(patch)));
...
export async function insertBooking(row) {
  const db = admin();
  if (!db) return { error: 'DB not configured' };
  let { data, error } = await db.from('bookings').insert(row).select('id').maybeSingle();
  if (holdSchemaError(error)) {
    ({ data, error } = await db.from('bookings').insert(withoutHoldColumns(row)).select('id').maybeSingle());
  }
  if (error) console.error('insertBooking:', error.message);
  // `conflict` = the slot is genuinely taken, as opposed to the database failing. Only a conflict
  // may release a customer's card hold (lib/holds.js releaseUnbookedHold) — an outage must not.
  const conflict = !!error && (error.code === '23P01' || /overlap|exclusion/i.test(error.message || ''));
  return { error: error?.message || null, conflict, id: (data && data.id) || null };
}

// ----- Card holds: authorise at booking, capture at check-in (migration 0034) -----------------
//
// The columns 0034 adds. Everything below exists so that a database which has NOT had 0034 pasted
// in yet still takes bookings: the hold columns are dropped from the write and the booking is
// saved without them. A booking that forgets it is a hold is bad; a customer whose slot vanished
// because a column was missing is worse, and the money is on the PaymentIntent either way.
const HOLD_COLUMNS = ['payment_state', 'authorized_cents', 'hold_expires_at', 'captured_at', 'released_at'];

function withoutHoldColumns(patch) {
  const out = { ...(patch || {}) };
  let dropped = false;
  for (const c of HOLD_COLUMNS) if (c in out) { delete out[c]; dropped = true; }
  if (dropped) console.warn('card holds: migration 0034 is not applied — the booking is saved without its hold state.');
  return out;
}

function holdSchemaError(error) {
  if (!error) return false;
  const msg = error.message || '';
  return HOLD_COLUMNS.some((c) => msg.includes(c)) && /does not exist|schema cache|column|constraint/i.test(msg);
}

// Everything a capture or a release has to weigh up: which PaymentIntent, what is on hold, when the
// hold dies, and what has already happened to it. `select('*')` on purpose — a named column list
// would be the one thing that breaks on a database still on 0033.
export async function bookingForHold(bookingId) {
  const db = admin();
  if (!db || !bookingId) return null;
  const { data, error } = await db.from('bookings').select('*').eq('id', bookingId).maybeSingle();
  if (error) { console.error('bookingForHold:', error.message); return null; }
  return data || null;
}

// Move a booking's money state. ONE writer for all four transitions, so the columns that must move
// together (payment_state with captured_at, with amount_cents) cannot drift apart across callers.
// `expect` makes it a compare-and-set: the update only lands while the row is still in the state the
// caller read, so two staff pressing Capture at the same moment cannot both win.
export async function setPaymentState({ bookingId, state, expect = null, patch = {} } = {}) {
  const db = admin();
  if (!db || !bookingId) return { error: 'DB not configured' };
  let q = db.from('bookings').update({ payment_state: state, ...patch }).eq('id', bookingId);
  if (expect === null) q = q.is('payment_state', null);
  else if (expect) q = q.eq('payment_state', expect);
  const { data, error } = await q.select('id');
  if (error) {
    if (holdSchemaError(error)) {
      console.warn(`setPaymentState(${state}): migration 0034 is not applied — the state was not written down.`);
      return { unsupported: true };
    }
...
// THE WARNING LIST. Capture is manual, so a hold nobody presses is money the venue simply never
// receives — this is the query that makes the forgotten ones visible. Ordered by hold_expires_at
// ASCENDING: the nearest cliff first, which is the order the portal should keep.
export async function heldBookings({ limit = 200 } = {}) {
  const db = admin();
  if (!db) return { rows: [] };
  const { data, error } = await db.from('bookings')
    .select('id,bay_id,booking_date,start_min,end_min,status,customer_name,customer_email,customer_phone,'
      + 'amount_cents,authorized_cents,hold_expires_at,payment_state,stripe_payment_intent')
    .eq('payment_state', 'held').neq('status', 'cancelled')
    .order('hold_expires_at', { ascending: true }).limit(limit);
  if (error) {
    if (missingSchema(error) || holdSchemaError(error)) {
      return { unsupported: true, rows: [],
        error: 'Card holds aren’t switched on in this database yet — apply migration 0034.' };
    }
    console.error('heldBookings:', error.message);
...
// ----- Proving who a request speaks for -----
// Anything that speaks for a customer starts from a PROVEN customer: a My Account session token
// — or, only on the local dev server, which sets req.devAccountPhone itself (server.js), the
// no-login dev account. A phone number typed at checkout is never enough.
...
// The venue does not offer saved cards. stripeCustomerIdFor(), checkoutStripeCustomer(),
// paymentElementSession(), listSavedCards(), removeSavedCard() and cardSetupIntent() lived here;
// nothing creates a Stripe Customer, opens a Customer Session, lists, adds or removes a card any
// more. The database side is untouched on purpose, the same treatment memberships (0028) and
// loyalty points got: customers.stripe_customer_id and its unique index still exist, holding the
// links already made, and no code in this codebase reads or writes that column. Apple Pay and
// Google Pay are unaffected — they come from automatic_payment_methods on the PaymentIntent
// (api/checkout.js), which never needed a Customer or a Customer Session.
...
// Records an obligation. MOVES NO MONEY BY ITSELF — and that is the point: this is step one of
// two. lib/refunds.js calls this FIRST, then Stripe, then settleRefund() below. So a row can exist
// without money having moved (an obligation somebody can see and chase), but money can never move
// without a row. Idempotent on (booking_id, ref); refuses to record more than was charged.
// Called on its own (api/booking-series.js ?action=cancel with refundMethod 'manual'), it is still
// what it always was: a note that somebody is owed something.
...
// Step two: what became of the money (migration 0033). 'succeeded' attaches the Stripe refund id
// and stamps settled_at; 'failed'/'cancelled' hand the reserved amount back to the booking so a
// refund that never moved stops counting against what is still refundable. Idempotent.
export async function settleRefund({ refundId, status, stripeRefundId = null, note = null } = {}) {
  const db = admin();
  if (!db) return { error: 'DB not configured' };
  if (!refundId) return { error: 'no_refund_id' };
  const { data, error } = await db.rpc('settle_refund', {
    p_refund: refundId, p_status: status, p_stripe_refund_id: stripeRefundId, p_note: note,
  });
  if (error) {
    if (missingSchema(error)) return seriesUnsupported('settleRefund', error);
    console.error('settleRefund:', error.message);
    return { error: error.message };
  }
  return data || { error: 'no_result' };
}

// One refund row, by id. Used to answer "has this one already been paid?" without re-deriving it.
export async function refundById(id) {
  const db = admin();
  if (!db || !id) return null;
  const { data, error } = await db.from('refunds').select('*').eq('id', id).maybeSingle();
  if (error) {
    if (missingSchema(error)) return seriesUnsupported('refundById', error);
    console.error('refundById:', error.message);
    return null;
  }
  return data || null;
}

// The row that carries a given Stripe refund. The webhook's first question: is this one of ours?
export async function refundByStripeId(stripeRefundId) {
  const db = admin();
  if (!db || !stripeRefundId) return null;
  const { data, error } = await db.from('refunds').select('*').eq('stripe_refund_id', stripeRefundId).maybeSingle();
  if (error) {
    if (missingSchema(error)) return seriesUnsupported('refundByStripeId', error);
    console.error('refundByStripeId:', error.message);
    return null;
  }
  return data || null;
}

// The row a refund request is about to be measured against: what was charged, what has already
// gone back, and which PaymentIntent to send it to.
export async function bookingForRefund(bookingId) {
  const db = admin();
  if (!db || !bookingId) return null;
  const { data, error } = await db.from('bookings')
    .select('id,group_id,bay_id,booking_date,start_min,end_min,status,amount_cents,refunded_cents,stripe_payment_intent,customer_name,customer_email,customer_phone')
    .eq('id', bookingId).maybeSingle();
  if (error) { console.error('bookingForRefund:', error.message); return null; }
  return data || null;
}

// A refund we recorded, sent to Stripe, and never managed to write the answer back for (the
// settle call failed, or the process died between the two). The webhook attaches the Stripe refund
// id to THIS row instead of writing a second one — otherwise a crash mid-refund would count the
// same money against the booking twice.
export async function pendingRefundFor({ bookingId, amountCents } = {}) {
  const db = admin();
  if (!db || !bookingId) return null;
  const { data, error } = await db.from('refunds').select('*')
    .eq('booking_id', bookingId).eq('amount_cents', Math.round(Number(amountCents) || 0))
    .eq('status', 'pending').is('stripe_refund_id', null).limit(1);
  if (error) {
    if (missingSchema(error)) return null;
    console.error('pendingRefundFor:', error.message);
    return null;
  }
  return (data && data[0]) || null;
}

// Which booking a Stripe charge belongs to. The webhook needs this to file a refund somebody
// issued from the Stripe dashboard against the right booking. Cancelled bookings count here —
// a refund almost always FOLLOWS a cancellation.
export async function bookingByPaymentIntent(piId) {
  const db = admin();
  if (!db || !piId) return null;
  // select('*') rather than a column list: the webhook also reads payment_state off this row to tell
  // "the authorisation just landed" from "the capture just landed", and a named list would be the
  // one thing that breaks on a database still on migration 0033.
  const { data, error } = await db.from('bookings')
    .select('*')
    .eq('stripe_payment_intent', piId).limit(1);
  if (error) { console.error('bookingByPaymentIntent:', error.message); return null; }
  return (data && data[0]) || null;
}
