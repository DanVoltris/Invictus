...
// The whole "what does this customer pay" calculation for the card path, in ONE place so the
// Express server and the Vercel function cannot drift apart.
//
// Order matters and is deliberate: the membership percentage comes off the list price first,
// then points burn against what is left. Points are stored value the shop controls, so spending
// them against an already-discounted total makes a member's points go further — the opposite
// order would quietly waste them.
export function quoteBooking({ settings, amountCents, plan, membershipExpires, todayISO, pointsBalance = 0, applyPoints = false }) {
  ...
  let charge = fullAmount - memberDiscountCents;
  ...
  return { fullAmount, memberPct, memberDiscountCents, pointsUsed, pointsDiscountCents, charge };
}
