...
// points as possible while leaving ≥ 50¢ to charge (Stripe's minimum).
export function pointsRedemption(settings, balance, amountCents) {
  ...
  if (bal >= toCover) return { pointsUsed: toCover, discountCents: amount, fullCover: true };
...
export function overrideEffects(overrides, settings, dateISO) {
  ...
  for (const o of overrides || []) {
    ...
    if (o.start_min == null) {
      ...
      // A multi-day block runs continuously: start time on the first day → end time on the
      // last day, with the days between blocked in full. Single-day = the plain range.
