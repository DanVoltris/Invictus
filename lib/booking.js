...
// Placeholder catalogue shown on the public /membership and /hours pages while Supabase
// is not configured, so the demo has something to show. These are NOT written anywhere —
// the moment the database is connected and seeded, the real rows take over and these stop
// being served. Purchase is blocked without a database (see the list handlers in api/).
export const DEMO_MEMBERSHIPS = [
  { id: 'demo-range', name: 'Range Pass', price_cents: 4900, period: 'month', discount_pct: 10,
    perks: '10% off every bay booking\nPriority booking window\nMember-only league nights',
    color: '#4AA3FF', sort: 1 },
  { id: 'demo-clubhouse', name: 'Clubhouse', price_cents: 9900, period: 'month', discount_pct: 15,
    perks: '15% off every bay booking\nTwo guest passes a month\nComplimentary coffee on every visit',
    color: '#F0523D', sort: 2 },
  { id: 'demo-elite', name: 'Invictus Elite', price_cents: 19900, period: 'month', discount_pct: 25,
    perks: '25% off every bay booking\nUnlimited guest passes\nAnnual club-fitting session\nReserved bay on request',
    color: '#14161A', sort: 3 },
];
...
//     → promo (% then $)   a price reduction — NOT YET IMPLEMENTED, see the note below
//     → floor at 0         never go negative before stored value is spent
//     → points             stored value: perishable, shop-controlled, worth nothing off-site
...
// what is left. Between the two kinds of stored value, POINTS BURN BEFORE GIFT CARDS: points are
// a marketing balance the venue can devalue or expire and that is worthless to anyone but this
// customer, while a gift card is a bearer instrument the venue owes on its books until it is
// spent. Burning the perishable, shop-controlled balance first leaves the customer holding the
// one that keeps its value, and clears the venue's liability no slower than the other order would.
//
// Promo codes are feature #9 in the build plan and land here later: a `promoDiscountCents`
// argument applied immediately after the membership percentage, before the floor. The floor and
// the two stored-value steps below are already written so that dropping it in changes nothing else.
export function quoteBooking({
  settings, amountCents, plan, membershipExpires, todayISO,
  pointsBalance = 0, applyPoints = false,
  ...
  let pointsUsed = 0, pointsDiscountCents = 0;
  if (applyPoints) {
    const r = pointsRedemption(settings, pointsBalance, charge, { partialOnly: true });
    pointsUsed = r.pointsUsed;
    pointsDiscountCents = r.discountCents;
    charge -= pointsDiscountCents;
  }

  // Gift card last. giftPartialOnly mirrors pointsRedemption's partialOnly and defaults to true
  // because this is the card path: a charge must remain for Stripe (50¢ minimum), so a card big
  // enough to cover everything still leaves 50¢ rather than producing a PaymentIntent Stripe
  ...
  return {
    ...
    pointsUsed, pointsDiscountCents,
...
export function membershipExpiryISO(period, fromISO) {
  ...
  // Clamp to the last day of the target month: Date.UTC(2026, 1, 31) silently rolls to Mar 3,
  // which handed anyone buying on the 29th-31st an extra month free.
  const lastDay = (yy, mm) => new Date(Date.UTC(yy, mm, 0)).getUTCDate();   // mm is 1-based
  const clamp = (yy, mm, dd) => {
    const yr = yy + Math.floor((mm - 1) / 12), mo = ((mm - 1) % 12) + 1;
    return at(yr, mo, Math.min(dd, lastDay(yr, mo)));
  };
  if (period === 'year') return clamp(y + 1, m, d);
  if (period === 'month') return clamp(y, m + 1, d);
  return null;   // 'once'
}
